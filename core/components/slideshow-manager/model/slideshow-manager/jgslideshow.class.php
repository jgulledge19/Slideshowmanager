...
class jgSlideshow {
    ...
    function __construct(modX &$modx,array $config = array()) {
    	$this->modx =& $modx;
        $corePath = $modx->getOption('slideshow-manager.core_path',null,$modx->getOption('core_path').'components/slideshow-manager/');
        $assetsPath = $modx->getOption('slideshow-manager.assets_path',null,$modx->getOption('assets_path').'components/slideshow-manager/');
        $assetsUrl = $modx->getOption('slideshow-manager.assets_url',null,$modx->getOption('assets_url').'components/slideshow-manager/');
        ...
        $this->modx->addPackage('slideshow-manager',$this->config['modelPath']);
        if ($this->modx->lexicon) {
            $this->modx->lexicon->load('slideshow-manager:default');
        }
    }

    /**
     * Initializes slideshow-manager based on a specific context.
     *
     * @access public
    ...
    public function initialize($ctx = 'mgr') {
        ...
        switch ($ctx) {
            case 'mgr':
                if (!$this->modx->loadClass('slideshow-manager.request.jgSlideshowControllerRequest',$this->config['modelPath'],true,true)) {
