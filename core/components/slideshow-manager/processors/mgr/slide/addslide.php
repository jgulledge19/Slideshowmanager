...
$validate->setFileRules('upload_file', $album_data['file_size_limit'], $file_allowed, 
        $album_data['file_width'], $album_data['file_height'], $this->jgSlideshow->config['uploadPath'].'tmp/' );
...
if( $validate->validate() ) {
    ...
    if( isset($_FILES['upload_file']['tmp_name']) && strlen($_FILES['upload_file']['tmp_name']) > 4 ) {
        // just the file name
        $input_data['file_path'] = str_replace( $this->jgSlideshow->config['uploadPath'], '', 
            $validate->moveFile('upload_file', $this->jgSlideshow->config['uploadPath'], 
...
$oldTarget = $modx->setLogTarget('HTML');
    if ( $slide->save() ) {
        ...
        // if repeating:
        if ( $edit_me ) {
            $message .= '<h3>The slide '.$slide->get('title').' has been updated.</h3>';
            
        } else {
            $message .= '<h3>The slide  '.$slide->get('title').' has been added.</h3>';
        }
        $this->action = 'slides';
        return $message;
    } else {
