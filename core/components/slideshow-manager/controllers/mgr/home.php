...
// load slideshow-manager lexicon settings:
$slide_lexicon = new dataToSmarty();

//print_r($tmp_sl);
//print_r($slide_lexicon); 
$my_lexicon = $slide_lexicon->smartyLexicon($modx->lexicon->fetch('slideshow_manager'));
...
foreach ($albums as $album_id => $album ) {
    ...
    <li>
        ...
        <p><a href="?a='.$a.'&amp;album_id='.$album->get('id').'&amp;action=slides">'.$my_lexicon['slideshow_manager']['manage'].'</a></p>
...
return $modx->smarty->fetch( MODX_CORE_PATH.'components/slideshow-manager/templates/album.tpl' );
