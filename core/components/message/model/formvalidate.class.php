...
class formValidate{
    ...
    public function checkFile($file){
        ...
        if ( !in_array($file_ext, $allow_array) || !$this->_validFileType($file_ext, $_FILES[$file]['type']) ) {
            if( $file_ext == 'rtf' && $_FILES[$file]['type'] == 'application/msword' ) {
                ...
                $this->error_array[$file] = 'Incorrect file type - ext: '.$file_ext.' mime type should be: '.implode(",", $this->allow_type).' but it is: '.$_FILES[$file]['type'];
                return;
            }
        }
    ...
    public function moveFile($file, $destination, $new_name) { //file is the name of the input feild, destination is full path with name
        if ( $this->isValid() && isset($this->allow_file_types[$file]['ext']) ) {
            ...
            //echo '<br>ORG: '.$org_file;
            //echo '<br>New: '.$new_file;
    ...
    /**
     * create a windows safe file name
     * @param name, string the desired filename
     * @param lower, boolean true makes the names lower
     */
    public function cleanName($name, $lower=true) {
        if ( $lower ) {
